...
class BinaryTree:
  ...
  def printTree(self, M, root, col, row, height):
    ...
    self.printTree(M, root.right, col + pow(2, height - 2), row + 1,
                   height - 1)
  ...
  def preOrdem(self):
    print("Pre Ordem")
    self._preOrdem(self.root)
    
  def emOrdem(self):
    print("Em Ordem")
    self._emOrdem(self.root)

  def posOrdem(self):
    print("Pos Ordem")
    self._posOrdem(self.root)

  def _emOrdem(self, node):
    if node.left:
      self._emOrdem(node.left)
    print(node)
    if node.right:
      self._emOrdem(node.right)

  def _preOrdem(self, node):
    print(node)
    if node.left:
      self._preOrdem(node.left)
    if node.right:
      self._preOrdem(node.right)

  def _posOrdem(self, node):
    if node.left:
      self._posOrdem(node.left)
    if node.right:
      self._posOrdem(node.right)
    print(node)
